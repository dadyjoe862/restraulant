...
function copy(value) {
  return Array.isArray(value) ? value.concat() : value !== null && typeof value === 'object' ?
  //
  // @ts-ignore
  Object.assign(new value.constructor(), value) : value;
}
function clean(part) {
  return isQuoted(part) ? part.substr(1, part.length - 2) : part;
}
function update(model, path, value) {
  let parts = _propertyExpr.default.split(path);
